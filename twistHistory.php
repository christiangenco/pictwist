...
<?php
	...
	// select current info
	// select parent id
	$query = "SELECT title, id, path, parent_photo_id FROM Photos WHERE id = ".$_REQUEST['p_id'].";";
	...
	{
		$query = "SELECT title, id, path FROM Photos WHERE id = ".$current_photo['parent_id'].";";
	...
	{
		echo '<a id="' . $parent_photo[p_id] . '" class="fancybox-iframe" rel="g1" href="'.$viewLightBoxURL.'?p_id=' . $parent_photo[p_id] . '">'.
